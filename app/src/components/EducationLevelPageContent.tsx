    ...
    <main className="overflow-hidden bg-white text-[#171b25]">
      <section className="relative border-b border-[#171b25]/15 bg-white">
        <div className="mx-auto max-w-7xl px-5 pb-10 pt-7 sm:px-8 sm:pb-12">
          ...
          >
            <div>
              <p className="font-mono text-[0.64rem] font-semibold uppercase tracking-[0.2em] text-[#139C48]">
              ...
              <h1 className="mt-5 max-w-[16ch] text-[3rem] font-bold leading-[1.08] tracking-[-0.04em] text-balance sm:text-[3.8rem] lg:text-[4.4rem]">
                ...
                <span className="mt-2 block">{levelDetails.hero.highlight}</span>
              </h1>
            </div>
            <div className="border-t border-[#171b25]/18 pt-6 lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group inline-flex min-h-12 items-center gap-3 bg-[#139C48] px-5 text-sm font-semibold text-white transition-colors hover:bg-[#0F7E3A] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#139C48]"
                >
                ...
                <a
                  href="#level-programs"
                  className="inline-flex min-h-12 items-center border border-[#171b25]/25 px-5 text-sm font-semibold text-[#171b25] transition-colors hover:border-[#139C48] hover:text-[#139C48] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#139C48]"
                >
      ...
      <section className="relative bg-[#f5f7f4] py-16 sm:py-20 lg:py-24">
        <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
          ...
          >
            <p className="font-mono text-[0.64rem] font-semibold uppercase tracking-[0.18em] text-[#139C48]">{levelDetails.overview.badge}</p>
            ...
            <p className="mt-8 border-l-2 border-[#139C48] pl-5 text-base font-semibold leading-7 text-[#414640]">{copy.pathway}</p>
          </motion.div>
          ...
          <div className="border-t-2 border-[#171b25]">
            {levelDetails.overview.points.map((point, index) => (
              ...
              >
                <span className="text-[2.7rem] font-bold leading-none tracking-[-0.06em] text-[#139C48]">{String(index + 1).padStart(2, "0")}</span>
                <div>
                  <p className="text-base font-semibold leading-7 text-[#313630]">{point}</p>
                  <span className="mt-4 block h-px w-12 bg-[#139C48]/50" />
                </div>
              </motion.article>
            ))}
          </div>
      ...
      <section id="level-programs" className="relative scroll-mt-24 bg-white py-16 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="grid gap-6 border-b-2 border-[#171b25] pb-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
            <div>
              <p className="font-mono text-[0.64rem] font-semibold uppercase tracking-[0.18em] text-[#139C48]">{shared.keyDetailsTitle}</p>
          ...
          {apiPrograms.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <div className={`relative min-h-64 overflow-hidden bg-[#fbf8f5] lg:col-span-5 lg:min-h-80 ${imageOnRight ? "lg:order-2" : ""}`}>
                      ...
                      />
                      <span className="absolute left-5 top-5 bg-white px-3 py-2 font-mono text-[0.6rem] font-semibold uppercase tracking-[0.12em] text-[#139C48]">
                    ...
                    <div className={`flex flex-col justify-between px-0 py-8 lg:col-span-7 lg:p-10 ${imageOnRight ? "lg:order-1" : ""}`}>
                      ...
                      <div className="mt-10 max-w-xl">
                        <h3 className="text-2xl font-bold leading-tight tracking-[-0.04em] transition-colors group-hover:text-[#139C48] sm:text-3xl">{title}</h3>
                        ...
                        <Link
                          ...
                          className="mt-7 inline-flex min-h-11 items-center gap-3 border-b border-[#139C48] text-sm font-semibold text-[#139C48] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#139C48]"
                        >
                          {copy.discover}
            ...
            <div className="space-y-12 pt-10">
              {fallbackCategories.map((category, categoryIndex) => (
                ...
                >
                  <div>
                    <span className="font-mono text-[0.62rem] font-semibold text-[#139C48]">{String(categoryIndex + 1).padStart(2, "0")}</span>
                    ...
                    {category.programs.map((program) => (
                      <article key={program.code} className="border-b border-[#dedad5] py-6">
                        <div className="flex flex-wrap items-start justify-between gap-4">
                          ...
                          <span className="font-mono text-[0.62rem] uppercase tracking-[0.12em] text-[#139C48]">{program.code}</span>
                        </div>
                        {program.description && <p className="mt-3 text-[0.78rem] leading-6 text-[#686c67]">{program.description}</p>}
                        {program.features && <p className="mt-4 border-l-2 border-[#139C48] pl-3 text-[0.68rem] leading-5 text-[#858984]">{program.features}</p>}
                      </article>
                    ))}
                  </div>
                </motion.div>
